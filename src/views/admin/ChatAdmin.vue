    ...
    <div class="container">
      <div class="row wrapper-content-chat">
        <div class="col-md-4 container-chat-left">
          ...
          <h6 class="text-center my-4">Chat from customer</h6>
          <div v-if="getlistStaffChat.length > 0">
            <div class="list-chat-customer" v-for="data in getlistStaffChat" :key="data.id" @click="pushChatRoom(data.id)">
              <div class="d-flex">
          ...
          <div class="no-customer" v-else>
            <h5>No Customer Asks</h5>
          </div>
        </div>
        <div class="col-md-8 container-chat-rigth">
          <div class="empty-chat" v-if="handleEmptyChat"><h3>please click one of the customer messages</h3></div>
...
export default {
  ...
  props: ['socket'],
  methods: {
    ...mapActions(['setListStaffChat', 'setHistoryChat', 'getUserById']),
    pushChatRoom (id) {
      this.handleEmptyChat = false
      this.setHistoryChat(id)
      this.getUserById(id)
      this.$router.push({ name: 'ChatRoomAdmin', params: { id } })
    },
    imgPlaceholder (e) {
      console.log(e)
